...
//
function queryDB(tx) {
    tx.executeSql('SELECT DISTINCT destination, city FROM bars WHERE statekey = ?', [selectedstate], querySuccess, errorCB);
}
...
function querySuccess(tx, results) {
    ...
    for (var i=0; i<len; i++){
        ...
        $('#state_list_ul').append('<li><a href="./profilechangebardest.html" data-transition="slide" rel="external" onClick="destClicked(\'' + bar.destination + '\')">' + bar.destination + ' - ' + bar.city + '</a></li>');
    }
